...
print(data_theta)
#diff_WQ_theta_data = tf.data.Dataset.from_tensor_slices((diff_WQ_theta_data_raw["Theta"], diff_WQ_theta_data_raw["WQ"]))
#diff_WQ_theta_data = tf.data.Dataset.from_tensor_slices(np.array(dict(diff_WQ_theta_data_raw)))
#diff_WQ_theta_data = tf.data.Dataset.from_tensor_slices(np.array(diff_WQ_theta_data_raw))
#diff_WQ_theta_data = tf.data.Dataset.zip()
#diff_WQ_theta_data = tf.data.Dataset.from_tensor_slices((data_theta, tf.reshape(tensor=tf.constant(diff_WQ_theta_data_raw["WQ"]), shape=[6000,1])))
...
#diff_WQ_theta_data_batches = tf.data.experimental.make_csv_dataset("diff_WQ_theta_data", batch_size = 10, label_name="WQ")
"""
...
"""
    for feature_batch in diff_WQ_theta_data.take(2):
        for key, value in feature_batch.items():
            print(" {!r:20s}: {}".format(key, value))
"""
