...
def main():
    ...
    eta_interval = np.linspace(start=-variables["eta_limit"], stop=variables["eta_limit"], num=nr_bins+1)
    # Crack values hereinbringen und werte im crack entfernen, array wieder ordnen
    eta_interval = np.concatenate((eta_interval, np.array([-1.52, -1.37, 1.37, 1.52])))
    eta_interval = eta_interval[(np.abs(eta_interval) >= 1.52) | (np.abs(eta_interval) <= 1.37)]
    ...
    for i,eta_value in enumerate(features_eta_constant.keys()):
        ...
        print(labels_eta_constant[eta_value])
        print(predictions[eta_value])
    ...
    print("eta", eta, len(eta))
    print("analytic integral in pb:", MC.gev_to_pb(analytic_integral), len(analytic_integral))
    ...
    analytic_stddev_mean = 1/np.sqrt(variables["total_data"]-1)*MC.gev_to_pb(analytic_stddev)
    analytic_stddev_mean = MC.gev_to_pb(analytic_stddev)
    ml_stddev_mean = 1/np.sqrt(variables["total_data"]-1)*MC.gev_to_pb(ml_stddev)
    ml_stddev_mean = MC.gev_to_pb(ml_stddev)
    print("stddev des Mittelwerts in pb:", analytic_stddev_mean)
    print("stddev des Mittelwerts in pb von ml", ml_stddev_mean)
    analytic_stddev = 0
    order = np.argsort(eta)
    eta = np.array(eta)[order]
    analytic_integral = np.array(analytic_integral)[order]
    WQ = integrate.trapezoid(y=MC.gev_to_pb(analytic_integral), x=eta)
    print("WQ integriert über eta:", WQ)
    ...
    ml.make_MC_plot(x=eta, analytic_integral=MC.gev_to_pb(analytic_integral), ml_integral=MC.gev_to_pb(ml_integral), xlabel=r"$\eta$",
                    ylabel=r"$\frac{d\sigma}{d\eta}$", save_path=save_path, name="xIntegration", analytic_errors=analytic_stddev_mean, ml_errors=ml_stddev_mean)
    plt.show()
