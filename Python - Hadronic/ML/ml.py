    ...
    for param in config_as_dict:
        if param in names_set:
            if type(config_as_dict[param]) == np.float64 or type(config_as_dict[param]) == np.int64:
                save_path += str(param) + "_" + "{:.2f}".format(config_as_dict[param]) + "_"
...
def construct_optimizer(optimizer, learning_rate=None, momentum=None, nesterov=None):
    try:
        optimizer = optimizer(learning_rate=learning_rate, momentum=momentum, nesterov=nesterov, clipvalue=1.5)
    except:
        try:
            optimizer = optimizer(learning_rate=learning_rate, clipvalue=1.5)
        except:
            optimizer = optimizer
    return optimizer
