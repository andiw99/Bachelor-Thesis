...
data_path = root_path + "/Files/Hadronic/Data/NewRandom/"
data_name = "all"
project_path = root_path + "Files/Hadronic/Models/training_schedule_comparison/"
loss_name = "best_loss"
project_name = ""
...
pools["min_delta"] = [5e-5, 5e-6]
pools["min_lr"] = [5e-7, 5e-8]
#Festlegen, welche Hyperparameter in der Bezeichnung stehen sollen:
names = {"min_delta", "min_lr"}
...
for config in checked_configs:
    ...
    actual_label_normalization = params["label_normalization"]
    actual_hidden_activation = params["hidden_activation"]
    if params["loss_fn"].name == "mean_absolute_error":
        min_delta = 1e-5
        params["label_normalization"] = actual_label_normalization
        ...
        min_delta = 1e-7
        params["label_normalization"] = False
    ...
    print("alle losses:", total_losses)
    print("total loss:", float(total_loss))

    # Modell und config speichern
    models[np.argmin(total_losses)].save(filepath=save_path, save_format="tf")
