...
import ml
#Modell laden 
hadronic_model = keras.models.load_model(filepath="FirstHadronicSafe/")
logarithm=False
...
for i, feature in enumerate(pred_feature_x_constant):
    ...
    train_losses_x_constant.append(float(loss_function((1/scaling)*hadronic_model(feature),hadronic_data_x_constant["WQ"][i])))
...
for i, feature in enumerate(pred_feature_eta_constant):
    ...
    train_losses_eta_constant.append(float(loss_function((1/scaling)*hadronic_model(feature),hadronic_data_eta_x_2_constant["WQ"][i])))
