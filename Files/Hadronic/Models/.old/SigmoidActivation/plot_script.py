...
import ml
#Modell laden 
hadronic_model = keras.models.load_model(filepath="SigmoidActivation/")
logarithm=True
scaling=10903854514176.0
...
for i, feature in enumerate(pred_feature_x_constant):
    ...
    train_losses_x_constant.append(float(loss_fn((1/scaling)*tf.math.exp(hadronic_model(feature)),hadronic_data_x_constant["WQ"][i])))
...
for i, feature in enumerate(pred_feature_eta_constant):
    ...
    train_losses_eta_constant.append(float(loss_fn((1/scaling)*tf.math.exp(hadronic_model(feature)),hadronic_data_eta_x_2_constant["WQ"][i])))
...
plt.plot(hadronic_data_x_constant["eta"], train_losses_x_constant)
plt.xlabel(r"$\eta$")
plt.ylabel("Loss")
plt.tight_layout()
plt.show()
...
plt.plot(hadronic_data_eta_x_2_constant["x_1"], train_losses_eta_constant)
plt.xlabel(r"$x_1$")
plt.ylabel("Loss")
plt.yscale("log")
plt.tight_layout()
