...
pools = dict()
pools["batch_size"] = [512, 128, 768, 256]
pools["learning_rate"] = [1e-3, 1e-4, 5e-3, 1e-5]
pools["fine_tuning_reduction"] = [1e-1, 1e-2, 1e-3]
...
pools["min_lr"] = [5e-8]
pools["source_model"] = ["best_model"]
pools["dataset"] = ["TransferData1M", ]
pools["rm_layers"] = [1, 2]
pools["add_layers"] = [0, 1, 2]
pools["units"] = [64, 128, 32, 16, 256]
pools["fine_tuning"] = [False, True]
pools["offset"] = [2, 4, 6]
# Festlegen, welche Hyperparameter in der Bezeichnung stehen sollen:
names = {"rm_layers", "add_layers", "units", "fine_tuning_reduction",
        "batch_size", "learning_rate", "fine_tuning", "offset"}

vary_multiple_parameters = True
grid_search = False

# Variablen...
train_frac = 1 # TODO train_frac so einstellen, dass man immer die gleiche anzahl an validation daten hat
validation_total = 0
size = 100
if grid_search:
    size = 1
...
for config in checked_configs:
    ...
    data_name = "all"
    test_data_path = root_path + "/Files/Transfer/Data/MMHT TestData50k/all"
    project_path = root_path + "Files/Transfer/Models/transferred_search_2/"
    ...
    (source_model, transformer) = ml.load_model_and_transformer(
    ...
    for i in range(repeat):
        ...
        (source_model, _) = ml.load_model_and_transformer(model_path=model_path)

        # Modell initialisieren
        models.append(
            ml.initialize_model(transfer=transfer, new_model=new_model,
        ...
        if params["fine_tuning"]:
            ...
            models[i].compile(optimizer=keras.optimizers.Adam(
                learning_rate=params["learning_rate"] * params["fine_tuning_reduction"],
                clipvalue=1.5),
                              loss=params["loss_fn"])
            print(models[i].summary())
            # history addieren? geht vermutlich nicht
            fine_tuning_history = models[i].fit(x=train_features,
                                                y=train_labels,
                                                batch_size=params[
                                                    "batch_size"],
                                                epochs=training_epochs,
            ...
            history = history.history["loss"] + fine_tuning_history.history[
                "loss"]
