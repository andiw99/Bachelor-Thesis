...
def main():
    ...
    time1 = time.time()
    #Daten einlesen
    data_path = "/home/andiw/Documents/Semester 6/Bachelor-Arbeit/Bachelor-Arbeit/Files/Partonic/PartonicData/TrainingData60k_ep_0.163/all"
    data_name = ""
    project_path = "/home/andiw/Documents/Semester 6/Bachelor-Arbeit/Bachelor-Arbeit/Files/Partonic/Models/PartonicTheta/"
    loss_name = "Source_loss"
    project_name = ""
    checkpoint_path = "/home/andiw/Documents/Semester 6/Bachelor-Arbeit/Bachelor-Arbeit/Files/Partonic/Models/VideoModels/"
    plot_path = "/home/andiw/Documents/Semester 6/Bachelor-Arbeit/Bachelor-Arbeit/Files/Partonic/PartonicData/PlottingData2k_ep_0.163/all"


    read_name = "video_test" #"best_guess_important_range" #
    label_name = "WQ"
    ...
    (_, plot_features, plot_labels, _, _, plot_features_pd, plot_labels_pd, _) = ml.data_handling(data_path=plot_path, label_name=label_name, return_pd=True, label_cutoff=False)
    ...
    for i in range(repeat):
        ...
        models.append(ml.initialize_model(nr_layers=nr_layers, units=units, loss_fn=loss_fn, optimizer=optimizer, learning_rate=learning_rate, hidden_activation=hidden_activation, output_activation=output_activation,
                                    kernel_initializer=kernel_initializer, bias_initializer=bias_initializer, l2_kernel=l2_kernel, l2_bias=l2_bias, dropout=dropout, dropout_rate=dropout_rate, momentum=momentum,
        ...
        # erste Instanz plotten
        keys = ml.get_varying_value(features_pd=plot_features_pd)
        predictions = {"Epoch 00": transformer.retransform(models[i].predict(transformer.rescale(plot_features)))}
        ml.plot_model(features_pd=plot_features_pd, labels=plot_labels, predictions=predictions, keys=keys,
                      save_path="/home/andiw/Documents/Semester 6/Bachelor-Arbeit/Bachelor-Arbeit/Files/Partonic/Models/VideoPictures/Epoch 00",
                      trans_to_pb=True, max_std=1e-10, format="png")
        plt.show()


    for i, model in enumerate(models):
        #callbacks initialisieren
        reduce_lr_on_plateau = keras.callbacks.ReduceLROnPlateau(monitor="loss", factor=lr_factor, patience=lr_patience,
                                                                 min_delta=min_delta, min_lr=min_lr)
        early_stopping = keras.callbacks.EarlyStopping(monitor="loss", min_delta=1e-1 * min_delta,
                                                       patience=stopping_patience)
        model_checkpoints = keras.callbacks.ModelCheckpoint(checkpoint_path  + "Epoch {epoch:03d}/", monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)
        lr_schedule = keras.callbacks.LearningRateScheduler(ml.class_scheduler(reduction=lr_reduction, min_lr=min_lr))
        callbacks = [reduce_lr_on_plateau, early_stopping, lr_schedule, model_checkpoints]
        #Training starten
        time4 = time.time()
    ...
    # Video Frames plotten
    print(sorted(os.listdir(checkpoint_path)))
    for checkpoint in sorted(os.listdir(checkpoint_path)):
        print(checkpoint)
        ml.save_config(new_model=new_model, model=model,
                       learning_rate=learning_rate,
                       training_epochs=training_epochs,
                       batch_size=batch_size, avg_total_Loss=avg_total_loss,
                       transformer=transformer,
                       training_time=training_time, custom=custom,
                       loss_fn=loss_fn, smallest_loss=smallest_loss,
                       loss_error=loss_error, total_losses=total_losses,
                       read_path=read_path, save_path=checkpoint_path + checkpoint)
        (checkpoint_model,
         checkpoint_transformer) = ml.load_model_and_transformer(
            model_path=checkpoint_path + checkpoint)

        # Für jedes Modell Predictions berechnen
        predictions = {checkpoint: checkpoint_transformer.retransform(checkpoint_model.predict(checkpoint_transformer.rescale(plot_features)))}
        ml.plot_model(features_pd=plot_features_pd, labels=plot_labels, predictions=predictions, keys=keys,
                      save_path="/home/andiw/Documents/Semester 6/Bachelor-Arbeit/Bachelor-Arbeit/Files/Partonic/Models/VideoPictures/" + checkpoint,
                      trans_to_pb=True, max_std=0.02, ratio_range=(0.99, 1.01), format="png")
        plt.show()
